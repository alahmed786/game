...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (tg) {
      ...
      app.ready();
      app.expand();
      // Closing confirmation was introduced in 6.2
        ...
        try {
            ...
            if (userResult.data) {
                ...
                const parsedPlayer: Player = {
                    ...
                    photoUrl: photoUrl || gamestate.photoUrl || row.photourl, 
                ...
                if (gamestate.upgrades) setUpgrades(gamestate.upgrades);
                ...
                if (startParam && startParam !== telegramId && loadedPlayer.invitedBy !== startParam) {
                     if (!loadedPlayer.invitedBy) {
                         processReferral(startParam, telegramId, adminConfig.referralRewardStars)
                            .catch(err => console.error("Referral Error:", err));
                         loadedPlayer.invitedBy = startParam;
                     }
                }
            ...
            const subscription = supabase
                ...
                .on(
                    'postgres_changes',
                    {
                        ...
                        schema: 'public',
                        table: 'players',
                        filter: `telegramid=eq.${telegramId}`,
                    },
                    (payload) => {
                        ...
                        setPlayer((prev) => {
                            ...
                            return {
                                ...prev,
                                stars: newRow.stars,
                                referralCount: newRow.referralcount,
                            };
                        });
                        if (newRow.referralcount > (playerRef.current?.referralCount || 0)) {
                             playStardustSound();
                        }
                    }
                )
  ...
  const savePlayerToSupabase = async (currentPlayer: Player, currentUpgrades: Upgrade[]) => {
      ...
      const fullGameState = { ...gameState, upgrades: currentUpgrades, photoUrl: currentPlayer.photoUrl };
      ...
      try {
          await supabase.from('players').upsert(dbPayload);
      } catch (err) {
  ...
  if (showIntro) {
      return <IntroScreen isDataReady={!!player} onFinished={() => setShowIntro(false)} isDarkMode={isDarkMode} />;
  }
