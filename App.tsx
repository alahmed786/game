...
const App: React.FC = () => {
    ...
    const initGame = async () => {
        ...
        try {
            ...
            if (userResult.data) {
                ...
                // OFFLINE EARNINGS FIX: Auto-Miner automatically unlocks offline earnings
                if (parsedPlayer.passivePerHour > 0) {
                    const now = Date.now();
  ...
  const buyUpgrade = (upgradeId: string) => {
    ...
    // Automatically turn on offline earnings if they buy the auto-miner
  ...
  const handleInitiateTask = (task: Task) => {
    ...
    if (task.type === 'telegram') {
        if (pendingTasks.includes(task.id)) {
            ...
            if (targetUrl) {
                 if (window.Telegram?.WebApp?.openTelegramLink) window.Telegram.WebApp.openTelegramLink(targetUrl); else if (window.Telegram?.WebApp?.openLink) window.Telegram.WebApp.openLink(targetUrl); else window.open(targetUrl, '_blank');
            }
        } return;
    }
    if (task.type === 'ads') {
