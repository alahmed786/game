...
const VerificationModal: React.FC<{ 
  ...
  return (
    <div className="fixed inset-0 bg-black/90 backdrop-blur-xl flex items-center justify-center z-50 p-6 animate-fade-in">
        <div className={`w-full max-w-sm bg-slate-950 border border-${theme}-500/30 rounded-3xl p-8 flex flex-col items-center gap-6 relative overflow-hidden shadow-2xl`}>
            ...
            <div className="flex flex-col items-center text-center gap-4 z-10">
                <div className={`w-20 h-20 rounded-full bg-gradient-to-b from-${theme}-900 to-black border border-${theme}-500/30 flex items-center justify-center mb-2 shadow-[0_0_30px_rgba(var(--bg-primary),0.2)]`}>
            ...
            <div className="flex items-center gap-2 w-full justify-center px-8">
                <div className={`h-1 flex-1 rounded-full transition-all duration-500 ${step >= 0 ? `bg-${theme}-500 shadow-[0_0_10px_rgba(var(--bg-primary),0.8)]` : 'bg-slate-800'}`}></div>
                <div className={`h-1 flex-1 rounded-full transition-all duration-500 ${step >= 1 ? `bg-${theme}-500 shadow-[0_0_10px_rgba(var(--bg-primary),0.8)]` : 'bg-slate-800'}`}></div>
            </div>

            {error && <p className="text-red-400 text-[10px] font-bold uppercase tracking-widest bg-red-950/50 px-3 py-1 rounded border border-red-500/20">{error}</p>}
...
const TransactionCard: React.FC<{ withdrawal: Withdrawal; isIndiaMode: boolean; theme: string }> = ({ withdrawal, isIndiaMode, theme }) => {
  ...
  const statusColor = 
    withdrawal.status === 'Paid' ? 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20' :
    withdrawal.status === 'Rejected' ? 'text-red-400 bg-red-500/10 border-red-500/20' :
    'text-amber-400 bg-amber-500/10 border-amber-500/20';

  return (
    <div className="relative group rounded-xl bg-slate-900/50 border border-white/5 p-4 transition-all hover:bg-slate-800/80 hover:border-white/10">
        <div className="flex justify-between items-start mb-3">
            <div className="flex items-center gap-3">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm border ${isUpi ? 'bg-orange-950/30 border-orange-500/20 text-orange-400' : 'bg-cyan-950/30 border-cyan-500/20 text-cyan-400'}`}>
                    {isUpi ? '₹' : '💎'}
                </div>
                <div>
                    <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{isUpi ? 'UPI Transfer' : 'TON Network'}</div>
                    <div className="text-[10px] text-slate-600 font-mono">{date} • {time}</div>
                </div>
            </div>
            <div className={`px-2 py-1 rounded text-[9px] font-bold uppercase tracking-wider border ${statusColor}`}>
                {withdrawal.status}
            </div>
        </div>
        ...
        <div className="flex justify-between items-end border-t border-white/5 pt-3">
            <div className="flex flex-col gap-1 max-w-[60%]">
                ...
                <span className="text-[10px] text-slate-400 font-mono truncate bg-black/20 px-1.5 py-0.5 rounded">
                    {withdrawal.address}
                </span>
            </div>
            <span className={`text-sm font-black font-mono ${isUpi ? 'text-orange-200' : 'text-cyan-200'}`}>
                {displayAmount}
            </span>
        </div>
    </div>
...
const WalletView: React.FC<WalletViewProps> = ({ player, onWithdraw, theme, minWithdrawal, onShowAd }) => {
  ...
  return (
    <div className="pt-6 px-4 pb-24 flex flex-col gap-8 min-h-full">
        {showVerify && (
            <VerificationModal 
                step={verifyStep} 
        ...
        <div className="flex items-center justify-between">
            <div>
                <h1 className="text-3xl font-black text-white tracking-tight">VAULT</h1>
                <div className="flex items-center gap-2 mt-1">
                    <div className={`w-2 h-2 rounded-full bg-${theme}-400 animate-pulse`} />
                    <span className={`text-[10px] font-bold text-${theme}-400 tracking-widest uppercase`}>
                        Secure Connection
                    </span>
                </div>
            </div>
            <div className="w-12 h-12 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-2xl backdrop-blur-sm shadow-lg">
                🏦
            </div>
        </div>

        {/* Balance Card - Quantum Node */}
        <div className="relative w-full aspect-[1.8] rounded-[2rem] overflow-hidden group shadow-2xl transition-transform duration-500">
            {/* Animated Background */}
            <div className={`absolute inset-0 bg-gradient-to-br from-${theme}-900 via-slate-900 to-black`}></div>
            ...
            <div className="absolute inset-0 p-7 flex flex-col justify-between z-10">
                <div className="flex justify-between items-start">
                    <div className="flex flex-col gap-1">
                        <span className="text-[10px] text-white/50 font-bold uppercase tracking-widest">Total Asset Value</span>
                    ...
                    <div className={`px-3 py-1.5 rounded-full bg-white/5 backdrop-blur-md border border-white/10 flex items-center gap-2 shadow-inner`}>
                        <span className="text-sm">{walletContext.icon}</span>
                ...
                <div className="space-y-4">
                    <div className="space-y-1.5">
                        <div className="flex justify-between text-[10px] font-bold text-white/50 uppercase tracking-wider">
                            ...
                            <span className={canWithdraw && !onCooldown ? `text-${theme}-300` : 'text-slate-400'}>
                        ...
                        <div className="h-2 w-full bg-black/40 rounded-full overflow-hidden border border-white/5">
                            ...
                            >
                                {canWithdraw && !onCooldown && <div className="absolute inset-0 bg-white/30 animate-pulse"></div>}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex justify-between items-center pt-2 border-t border-white/5">
                        <div className="flex flex-col">
                            <span className="text-[9px] text-white/30 font-bold uppercase">Network</span>
                            <span className="text-[10px] text-white/80 font-mono tracking-wide">{walletContext.network}</span>
                        </div>
                        <div className="flex flex-col items-end">
                            <span className="text-[9px] text-white/30 font-bold uppercase">Min. Limit</span>
                            <span className="text-[10px] text-white/80 font-mono tracking-wide">{minWithdrawalDisplay}</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-4">
                ...
                <div className="relative group">
                    <div className={`absolute -inset-0.5 bg-gradient-to-r from-${theme}-500 to-blue-600 rounded-2xl opacity-0 group-hover:opacity-30 group-focus-within:opacity-50 transition duration-500 blur`}></div>
                    <div className="relative bg-slate-900 border border-white/10 rounded-2xl p-1 flex items-center shadow-lg transition-all group-focus-within:bg-slate-900/80">
                        <div className="pl-4 pr-3 text-slate-500 text-xl transition-colors group-focus-within:text-white">
                            {isIndiaMode ? '💳' : '⛓️'}
                        </div>
                        <input 
                            value={address}
        ...
        <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-3">
                {player.withdrawalHistory.length > 0 ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-12 border border-dashed border-slate-800 rounded-2xl bg-slate-900/30 text-slate-600 gap-3">
