...
const AdDealModal: React.FC<{ deal: StellarDeal; onConfirm: () => void; onCancel: () => void; theme: string; onShowAd: UpgradesViewProps['onShowAd'] }> = ({ deal, onConfirm, onCancel, theme, onShowAd }) => {
  ...
  return (
    <div className="fixed inset-0 bg-white/60 dark:bg-black/60 backdrop-blur-md flex items-center justify-center z-50 p-4">
      <div className="w-full max-w-sm bg-white/60 dark:bg-black/50 backdrop-blur-xl border border-white/30 dark:border-white/10 rounded-2xl p-6 flex flex-col items-center gap-4 text-center shadow-2xl">
        <span className="text-5xl">{deal.icon}</span>
        <h2 className="text-xl font-bold text-slate-900 dark:text-white">{deal.title}</h2>
        <p className="text-slate-600 dark:text-slate-300 text-sm">{deal.description}</p>
...
const StellarDealCard: React.FC<{ deal: StellarDeal, player: Player, onBuy: (deal: StellarDeal) => void, theme: string }> = ({ deal, player, onBuy, theme }) => {
  ...
  const getButtonContent = () => {
    if (activeBoost) return <><span className="text-green-600 dark:text-green-400">Active:</span><span className="font-mono text-slate-900 dark:text-white">{formatTime(timeLeft)}</span></>;
    if (onCooldown) return <span className="font-mono text-slate-900 dark:text-white">{formatTime(timeLeft)}</span>;
    if (!isUnlocked) return <span className="text-red-500 dark:text-red-400">LVL {deal.unlockLevel} REQ.</span>;
    
    return <><span>{costIcon}</span><span className={deal.costType === 'ad' ? 'text-slate-900 dark:text-white' : ''}>{deal.costType === 'ad' ? 'Watch Ad' : deal.cost.toLocaleString()}</span></>;
  };

  const forbiddenTechStyles = isForbiddenTech 
    ? "bg-gradient-to-br from-yellow-200/40 via-purple-200/40 to-white/60 dark:from-yellow-900/40 dark:via-purple-900/40 dark:to-slate-900/60 border-yellow-500/50 shadow-[0_0_20px_rgba(234,179,8,0.2)] animate-pulse"
    : `bg-white/60 dark:bg-slate-900/60 border-${activeBoost ? 'green-500' : 'yellow-500/30'}`;

  return (
    <div className={`relative w-full backdrop-blur-xl border rounded-2xl p-4 flex flex-col justify-between gap-4 transition-all shadow-lg
    ...
    `}>
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-3">
          <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-2xl border shadow-inner ${isForbiddenTech ? 'bg-gradient-to-br from-yellow-400 to-amber-500 border-yellow-300 text-white' : 'bg-white/80 dark:bg-slate-800 border-white/20 dark:border-slate-700'}`}>
              {deal.icon}
          </div>
          <div>
            <h3 className={`font-bold text-base ${isForbiddenTech ? 'text-slate-900 dark:text-yellow-100' : 'text-slate-900 dark:text-white'}`}>{deal.title}</h3>
            <p className={`text-xs leading-tight ${isForbiddenTech ? 'text-slate-600 dark:text-yellow-200/70' : 'text-slate-600 dark:text-slate-300'}`}>{deal.description}</p>
          </div>
        </div>
      </div>
      <button
        ...
        className={`w-full py-2.5 rounded-xl font-bold text-xs uppercase tracking-widest flex items-center justify-center gap-2 transition-all disabled:cursor-not-allowed
            ${canBuy && deal.costType === 'ad' ? `bg-${theme}-500 text-white shadow-lg` : canBuy ? 'bg-white dark:bg-slate-100 text-slate-950 shadow-lg border border-slate-200 dark:border-transparent' : 'bg-slate-200 dark:bg-slate-800 text-slate-500 dark:text-slate-600'}
        ...
        `}
        style={onCooldown ? { background: 'repeating-linear-gradient(-45deg, #cbd5e1, #cbd5e1 10px, #e2e8f0 10px, #e2e8f0 20px)' } : {}}
      >
        {getButtonContent()}
      </button>
    </div>
