...
const TON_TO_INR_RATE = 640; // Defined locally to match Wallet logic
...
const AdminView: React.FC<AdminViewProps> = ({
  ...
  const addDeal = () => {
    ...
    const deal: StellarDeal = {
        ...
        rewardValue: Number(newDeal.rewardValue) || 1000, // simplified for demo
  ...
  // --- Finance ---
  const handleWithdrawalAction = (id: string, action: 'Paid' | 'Rejected') => {
    setWithdrawals(prev => prev.map(w => w.id === id ? { ...w, status: action } : w));
  };

  // --- Users ---
  const toggleBan = (id: string) => {
    setPlayers(prev => prev.map(p => p.telegramId === id ? { ...p, isBanned: !p.isBanned } : p));
  };

  return (
    <div className="fixed inset-0 bg-slate-950 z-[200] overflow-y-auto text-white font-mono">
      <div className="bg-slate-900 border-b border-red-900 p-4 sticky top-0 z-10 flex justify-between items-center">
      ...
      <div className="flex overflow-x-auto border-b border-slate-800 bg-slate-950 sticky top-[73px] z-10">
